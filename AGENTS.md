...
This repository builds study courses and the site that reads them. Follow the workflow below for course work; otherwise read `README.md` and `docs/`.

# Authoring a course

Write the course in this conversation. From the repository root, `node tools/author.mjs` provides rules, sources, progress, and non-blocking warnings; it never runs a model.
...
- Request each rule set once; after compaction use `node tools/author.mjs status <id> --digest`.
...
- Resume with `node tools/author.mjs begin <id>`; recorded work is not redone.
- Revise with `node tools/author.mjs redo <id> <sN-M>...` (or `course`, `finish`), edit those files, and run `done` again for each.
- Read a course with `node tools/pack.mjs <id>`; it writes `packed/<id>.course.json` for the study site.
