...
import { DepthTab } from "./TierStub.jsx";
...
function usePair() {
  ...
  const track = e => {
    ...
    if (!el) return; // Focus-out can arrive after the row was unmounted.
    const target = e.type === "mouseout" || e.type === "focusout" ? e.relatedTarget : e.target;
    const pair = target?.closest?.("[data-xr]");
    const key = pair && el.contains(pair) ? pair.getAttribute("data-xr") : null;
    el.querySelectorAll("[data-xr]").forEach(n =>
      n.classList.toggle("hot", key != null && n.getAttribute("data-xr") === key));
  };
  return { ref, over: track, out: track };
}
...
function Blocks({ sub, ctx, lane, depth, expandAll, openAt }) {
  ...
  /* All tiers use the same renderer and margin notes. */
  const fullRow = ({ b, i }) => {
    const refs = refsOf(b);
    const notes = (
      <>
  ...
  const compactRow = (it, p, children) => (
    <PairRow key={it.i} id={blockId(sub.id, it.i)} cls="brow nbrow"
      side={<Asides b={it.b} ctx={ctx}
        paired={new Set(p.mode === "lead" ? anchorKeys(p.lead) : [])}
        onOpen={() => openBlock(it.i)} />}>
      {children}
    </PairRow>
  );
  ...
  const drawRun = items => {
    ...
    return topics.map((topic, t) => {
      ...
      const draw = it => {
        ...
        if (p.mode === "hidden" && !forced) {
          if (asidesOf(b).length) rows.push(compactRow(it, p, null));
          return;
        }
        ...
        rows.push(
          compactRow(it, p,
            <NoteRow b={b} p={p} ctx={ctx} refs={refsOf(b)} showCat={showCat}
      ...
      if (head && headP.mode !== "full") {
        ...
        rows.push(headOpen ? heading : compactRow(head, headP, heading));
      } else if (head) {
  ...
  return runs.map(run => {
    ...
    if (!run.hidden) return compact ? drawRun(run.items) : run.items.map(it => fullRow(it));

    const addressed = run.items.some(it => it.i === openAt);
    const shown = expandAll || (openRun[at] ?? addressed);
    const panelId = `${sub.id}-depth-${at}`;
    // Explicitly revealing a tier shows its full blocks, including graphics,
    // even when surrounding content is in Review. The tab stays to close it.
    return (
      <div class="depth-run" key={panelId}>
        {!expandAll && <PairRow cls="brow depth-control" side={
          <DepthTab items={run.items} open={shown} controls={panelId}
            onToggle={() => setOpenRun(o => ({ ...o, [at]: !shown }))} />
        } />}
        <div id={panelId} hidden={!shown}>
          {shown && run.items.map(it => fullRow(it))}
        </div>
      </div>
    );
  });
...
/* A compact block keeps its notes beside its own text. */
function PairRow({ cls, id, side, children }) {
  const pair = usePair();
  return (
    <div class={cls} id={id} ref={pair.ref}
         onMouseOver={pair.over} onMouseOut={pair.out}
         onFocusIn={pair.over} onFocusOut={pair.out}>
