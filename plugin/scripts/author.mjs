...
const fail = (s) => {
	console.log(s);
	try {
		if (existsSync(courseDir)) note(courseDir, `\`author ${process.argv.slice(2).join(" ")}\` → ${s}`);
	} catch {}
	process.exit(1);
};
...
const commands = {
	begin() {
		...
		for (const rel of left) {
			...
			for (let d = dirname(join(courseDir, rel)); d !== courseDir && !readdirSync(d).length; d = dirname(d)) rmSync(d, { recursive: true });
		}
	...
	redo() {
		if (!positional.length || positional.some((r) => !/^(s\d+-\d+|course|finish)$/.test(r))) fail(`usage: author redo ${id} <sN-M|course|finish>...`);
		const d = digest(courseDir);
