...
const SKIP = /* @__PURE__ */ new Set(["sources", "node_modules"]);
...
//#endregion
//#region tools/pack.mjs
join(dirname(fileURLToPath(import.meta.url)), "..");
const COURSES = COURSES$1;
const OUT = join(WORKSPACE, "packed");
const args = process.argv.slice(2);
const all = readdirSync(COURSES, { withFileTypes: true }).filter((d) => d.isDirectory() && !d.name.startsWith("_")).map((d) => d.name);
let ids;
if (args.includes("--private")) ids = all.filter((id) => !PUBLIC.has(id));
else {
...
if (unknown.length) {
	console.error(`no such course: ${unknown.join(", ")}`);
	process.exit(1);
}
...
for (const id of ids) {
	...
	const files = courseFiles(join(COURSES, id), (m) => {
		console.error(`${id}: ${m}`);
		process.exit(1);
	});
	...
	console.log(`  ${id.padEnd(12)} ${String(Object.keys(files).length).padStart(3)} files · ${(statSync(out).size / 1024).toFixed(0)}KB · ${(gzipSync(body).length / 1024).toFixed(0)}KB gzipped -> packed/${id}.course.json`);
}
