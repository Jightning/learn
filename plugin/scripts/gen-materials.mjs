...
//#endregion
//#region tools/gen-materials.mjs
...
function checklist(C) {
	...
	for (const s of C.sections) {
		...
		s.subs.forEach((u, k) => {
			const n = `${s.num}.${k + 1}`;
			for (const q of u.quiz || []) out.push(`- [ ] ${q.type}, [${n}](../index.html#${u.id})`);
		});
		out.push("");
	}
	...
	if (keys.length) {
		...
		for (const k of keys) out.push(`- [ ] ${(C.concepts[k] || {}).term || k}: durable, not merely answered ([drill](../index.html#/${C.id}/c/${k}))`);
		out.push("");
	}
	return out.join("\n");
}
const ids = readdirSync(COURSES, { withFileTypes: true }).filter((d) => d.isDirectory() && !d.name.startsWith("_")).map((d) => d.name).filter((n) => !wanted.length || wanted.includes(n));
let stale = 0;
let wrote = 0;
for (const id of ids) {
	const { course: C } = loadCourse(join(COURSES, id));
	if (!Object.keys(C.drills || {}).length) continue;
	C.id = id;
	for (const [name, body] of [["problems.md", problems(C)], ["checklist.md", checklist(C)]]) {
		const path = join(COURSES, id, "materials", name);
		if ((existsSync(path) ? readFileSync(path, "utf8") : null) === body) continue;
		if (CHECK) {
