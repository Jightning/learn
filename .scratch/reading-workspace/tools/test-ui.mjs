...
/* ---------------------------------------------------------------------------
 * An aside's anchor carries no mark at rest.
 *
 * It used to take a dotted underline, on the argument that a concept mention
 * takes one — but a concept mention is a destination and earns its mark by
 * going somewhere, while an anchor goes nowhere: its note is already beside it.
 * What the rule bought was a second texture mid-sentence for every reader who
 * did not want the note. The pairing on hover is the whole device now, so the
 * rule is that the phrase is indistinguishable from its sentence until then.
 * ------------------------------------------------------------------------ */
if (ids.includes("demo")) {
  await go("#/demo/s5-1");
  if (await page.locator(".nref").count()) {
    const rest = await page.evaluate(() => {
      const c = getComputedStyle(document.querySelector(".nref"));
      return { border: c.borderBottomStyle, bg: c.backgroundColor, ink: c.color };
    });
    ck("an aside's anchor is unmarked until it is wanted",
       rest.border === "none" && /rgba\(0, 0, 0, 0\)|transparent/.test(rest.bg),
       JSON.stringify(rest));
  }
}

/* ---------------------------------------------------------------------------
 * Changing the width must not move the reader.
 *
 * lib/place.js re-pins on a resize, and used to pin the subsection — which is
 * the right thing to *write down* for a relaunch and much too coarse for this.
 * A subsection runs past 2000px; narrowing the window adds a third to the
 * height of everything inside it, so restoring its top still left the reader a
 * screen away from the line they were on, which is the scroll this exists to
 * save them. The anchor for a reflow is the block row.
 * ------------------------------------------------------------------------ */
{
  await go("#/" + ids[0]);
  const sec = await page.evaluate(() => {
    const a = document.querySelector(".toc a[href]");
    return a && a.getAttribute("href");
  });
  if (sec) {
    await go(sec);
    /* A real reading position: a block row just above the fold, well down a
       page long enough for the re-wrap above it to matter. */
    const at = await page.evaluate(() => {
      const rows = [...document.querySelectorAll(".brow[id]")];
      const el = rows[Math.floor(rows.length * 0.6)];
      if (!el) return null;
      scrollTo({ top: scrollY + el.getBoundingClientRect().top + 8, behavior: "instant" });
      return el.id;
    });
    if (at) {
      /* Past the sampler: the anchor has to be taken before the reflow, which
         is why place.js samples on every scroll frame rather than reading the
         position when a resize arrives. */
      await page.waitForTimeout(1400);
      const top = id => page.evaluate(
        i => Math.round(document.getElementById(i).getBoundingClientRect().top), id);
      const before = await top(at);
      await page.setViewportSize({ width: 900, height: 900 });
      await page.waitForTimeout(1000);
      const after = await top(at);
      ck("a change of width keeps the line the reader was on",
         Math.abs(after - before) <= 12, `${before}px -> ${after}px`);
      await page.setViewportSize({ width: 1440, height: 900 });
      await page.waitForTimeout(600);
    }
  }
