 ...
 * is held *across a reflow* is a character in the visible line, with a block
 * fallback for graphics. `land` reapplies it as layout finishes arriving.
 * ==========================================================================*/
...
/* Hold a character on the first visible reading line, rather than a pixel
 * offset inside a paragraph whose wrapping is about to change. A live Range
 * follows the same text through layout without editing the content DOM. */
const readingTop = () => Math.max(0, document.querySelector(".topbar")?.getBoundingClientRect().bottom || 0) + 12;

function readingAnchor() {
  const y = readingTop();
  for (const main of document.querySelectorAll(".sub .bmain")) {
    const box = main.getBoundingClientRect();
    if (box.top > y || box.bottom <= y || box.width === 0) continue;
    const x = Math.max(0, box.left) + Math.min(24, box.width / 2);
    ...
    if (node?.nodeType !== Node.TEXT_NODE || !main.contains(node) || !node.length) continue;
    // SVG labels and the hidden accessibility copy of maths aren't prose.
    if (node.parentElement.closest("svg, .katex, button:not(.nrow-b):not(.ntopic-b), .note-pull")) continue;
    const range = document.createRange();
    const start = Math.min(offset, node.length - 1);
    range.setStart(node, start);
    range.setEnd(node, start + 1);
    const rect = range.getBoundingClientRect();
    if (!rect.height || Math.abs(rect.top - y) > 48) continue;
    return { el: range, top: rect.top - y, text: true };
  }
  // Figures, gaps, and quizzes still have stable block or subsection elements.
  const el = lastAbove(".sub[id], .brow[id]");
  return el ? { el, top: el.getBoundingClientRect().top } : { el: null, top: -scrollY };
}
...
export function repin() {
  if (!at) return;
  if (at.el) {
    const connected = at.text ? at.el.commonAncestorContainer.isConnected : at.el.isConnected;
    if (connected) land(at.el, -(at.top + (at.text ? readingTop() : 0)));
...
export function track() {
  ...
  const sample = () => {
    raf = 0;
    // Resize-triggered scroll events must not overwrite the pre-reflow anchor.
    if (innerWidth !== w || landing) return;
    at = readingAnchor();
  };
