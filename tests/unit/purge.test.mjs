...
const { writeNote, readNote, readNotes, writeNotes, setFolded, isFolded } = await import("../../src/lib/notes.js");
...
function live(C) {
  ...
  writeNotes(C.id, "s1-2#4", [""]);        /* a blank note is the saved marker */
...
const held = C => ({
  ...
  saved: readNotes(C.id, "s1-2#4").some(t => !t.trim()),
